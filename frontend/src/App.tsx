...
type RunDraft = {
  ...
  batchSize: string;
  maxNewTokens: string;
  thinkingMode: "model_default" | "disabled" | "enabled";
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_19rem]">
        ...
        <aside className="space-y-5">
          ...
          {job?.progress.message ? <div className="rounded-xl border border-line bg-white/[0.03] p-3" role="status"><div className="flex items-center justify-between gap-3 text-xs"><span className="leading-5 text-muted">{job.progress.message}</span><span className="shrink-0 font-mono text-[0.62rem] text-cyan">{job.progress.total ? `${job.progress.completed}/${job.progress.total}` : job.progress.stage.replaceAll("_", " ")}</span></div>{job.progress.total ? <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/[0.06]" aria-label="Model download progress"><div className="h-full rounded-full bg-cyan transition-all" style={{ width: `${Math.min(100, (job.progress.completed / job.progress.total) * 100)}%` }} /></div> : null}</div> : null}
...
function RunConfigPage({ onNavigate }: { onNavigate: (item: NavigationItem) => void }) {
  ...
  const replicationError = validateDatasetSeed(run.replication);
  ...
  const ready = !modelError && !datasetError && !sampleError && !seedError && !replicationError && !batchError && !tokenError && !runNameError;
  const running = job?.state === "queued" || job?.state === "running";
        ...
        <main className="space-y-5">
          ...
          <section className="research-card">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <label className="field-label" htmlFor="replication">Replication<input id="replication" className={`input-control mt-2 ${replicationError ? "input-control-error" : ""}`} value={run.replication} onChange={(event) => update("replication", event.target.value)} inputMode="numeric" /></label>
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5"><p className={`field-hint ${sampleError ? "field-hint-error" : ""}`}>{sampleError ?? "Rows are bounded before execution."}</p><p className={`field-hint ${seedError ? "field-hint-error" : ""}`}>{seedError ?? "The same revision and seed select the same rows."}</p><p className={`field-hint ${replicationError ? "field-hint-error" : ""}`}>{replicationError ?? "Increment this for an independent run over identical rows."}</p><p className={`field-hint ${batchError ? "field-hint-error" : ""}`}>{batchError ?? "Schedule remains deterministic."}</p><p className={`field-hint ${tokenError ? "field-hint-error" : ""}`}>{tokenError ?? "Generation budget is recorded."}</p></div>
            ...
            <div className="mt-6"><span className="field-label">Thinking mode</span><div className="mt-2 inline-flex rounded-xl border border-line bg-ink p-1" role="group" aria-label="Thinking mode"><button type="button" className={`runner-tab ${run.thinkingMode === "disabled" ? "runner-tab-active" : ""}`} aria-pressed={run.thinkingMode === "disabled"} disabled={run.mode !== "generation"} onClick={() => update("thinkingMode", "disabled")}>Disabled</button><button type="button" className={`runner-tab ${run.thinkingMode === "enabled" ? "runner-tab-active" : ""}`} aria-pressed={run.thinkingMode === "enabled"} disabled={run.mode !== "generation"} onClick={() => update("thinkingMode", "enabled")}>Enabled</button><button type="button" className={`runner-tab ${run.thinkingMode === "model_default" ? "runner-tab-active" : ""}`} aria-pressed={run.thinkingMode === "model_default"} disabled={run.mode !== "generation"} onClick={() => update("thinkingMode", "model_default")}>Model default</button></div><p className="field-hint mt-2">Explicit control uses the tokenizer’s chat template. Unsupported tokenizers fail before producing evidence.</p></div>
